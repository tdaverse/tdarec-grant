...
Its author Dr. Islambekov, together with Dr. Luchinsky and others, describe the proposed persistence block vectorization and its methodological potential in recent manuscripts [@chan2022computationally; @islambekov2023vector].
...
The {TDAvec} package is not designed to interface with other TDA packages except {TDA} and could not be conveniently built in to a Tidymodels workflow.
...
The final data structure must be able to store extended persistent homology [@cohensteiner2009extending], in which each feature falls into one of three bins: ordinary, relative, and extended. While ordinary and relative features are distinguished by whether each feature's birth value is smaller (ordinary) or larger (relative) than its death value, extended features may satisfy either inequality and must be distinguished.
