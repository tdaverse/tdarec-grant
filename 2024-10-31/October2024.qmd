...
The [{TDAvec}](https://cran.r-project.org/package=TDAvec) package is not designed to interface with other TDA packages except [{TDA}](https://cran.r-project.org/package=TDA) and could not be conveniently built in to a [tidymodels](https://www.tidymodels.org) workflow.
...
- _Aim 1.2_: Develop and publish the [{recipes}](https://recipes.tidymodels.org) extension [{tdarec}](https://github.com/corybrunson/tdarec) with [{TDAvec}](https://cran.r-project.org/package=TDAvec) its primary dependency for vectorizations of persistent homology (PH).
:::
...
Because we have not yet made detailed plans for [{TDAvec}](https://cran.r-project.org/package=TDAvec) with its authors, the remainder of this section will discuss only [{tdarec}](https://github.com/corybrunson/tdarec).

#### Prototype [{recipes}](https://recipes.tidymodels.org) extension
...
Because [{TDAvec}](https://cran.r-project.org/package=TDAvec) uses rigorous naming and parameterizing standards, we plan to write a helper script to generate the source code of one vectorization step for each function `TDAvec::compute*()`, tailored to that function's arguments and derived in part from [{TDAvec}](https://cran.r-project.org/package=TDAvec) documentation ([#3](https://github.com/corybrunson/tdarec/issues/3)).
This will allow to rapidly update [{tdarec}](https://github.com/corybrunson/tdarec) as new vectorizations are implemented in [{TDAvec}](https://cran.r-project.org/package=TDAvec), both during development and between releases.
...
The [{recipes}](https://recipes.tidymodels.org) extension [{tdarec}](https://github.com/corybrunson/tdarec) will `Suggest` [{ripserr}](https://tdaverse.github.io/ripserr/) as an engine for computing PH of point clouds (`step_phom_point_cloud()`) and of images (`step_phom_image()`).
While it will be the only engine for the latter, other engines will be available for the former, including those of [{TDA}](https://cran.r-project.org/package=TDA), which will also be `Suggest`ed.

Because engine deployment for computing PH is required by at least two [tdaverse](https://github.com/tdaverse/) packages ([{ggtda}](https://tdaverse.github.io/ggtda/) and [{tdarec}](https://github.com/corybrunson/tdarec)), deployment functions may be included in the helper package described in the section on Aim 2.
...
- Add missing features in [{flipr}](https://lmjl-alea.github.io/flipr/): ANOVA, Friedman-Lane and other permutation schemes for regression, etc.;
- Put more thinking into confidence regions (computational cost in particular);
- Improve documentation;
- Improve test coverage.

AST will supervise the work of MSI and focus on the development of two new packages for enable inference for topological data, namely {phutil} and {inphr}. In detail, AST will contribute the following points:
...
- write, document, test and publish a lightweight package {phutil} (stands for **P**ersistence **H**omology **UTIL**ities and should be pronounced *futile*), for a [tdaverse](https://github.com/tdaverse)-opinionated data structure that stores PH data and a number of utility functions to be used by reverse dependencies;
...
The scope of the package is to implement a lightweight data structure to host a persistence diagram and a number of utility functions to be used by reverse dependencies. There is an [issue](https://github.com/tdaverse/tdaverse/issues/4) on the [{tdaverse}](https://github.com/tdaverse/tdaverse) repository to brainstorm for the best representation.
...
Using both {phutil} which will provide a class for a single persistence diagram and [{flipr}](https://lmjl-alea.github.io/flipr/) which provides the permutation mechanisms, {inphr} will provide:
...
In order to most faithfully and efficiently join the current release of [Ripser](http://ripser.org/) (`main` branch) with features implemented in development branches, three computer and information science and engineering (CISE) students will build proficiency in the essential algorithms of PH and the [Ripser](http://ripser.org/) code base under JBR's supervision, then collaborate with JBR to upgrade the [{ripserr}](https://tdaverse.github.io/ripserr/) package, probably by rebuilding from scratch with the existing version as a guide[^interviews].
...
We have charted a plan for a 3-month pay period to complete the upgrade, documentation, and re-release of [{ripserr}](https://tdaverse.github.io/ripserr/), with the following monthly products of work:

1. Installable upgrade of [{ripserr}](https://tdaverse.github.io/ripserr/) using the current [Ripser](http://ripser.org/) code base;
...
As detailed under Specific Aim 1, [{ripserr}](https://tdaverse.github.io/ripserr/) will be `Suggest`ed by [{tdarec}](https://github.com/corybrunson/tdarec) as well as other packages like [{ggtda}](https://tdaverse.github.io/ggtda/).
