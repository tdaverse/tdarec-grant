...
Refactor {ripserr} with a current implementation of [Ripser](http://ripser.org/) and connect additional options to R.
:::
...
The {TDAvec} package is not designed to interface with other TDA packages except {TDA} and could not be conveniently built in to a [tidymodels](https://www.tidymodels.org) workflow.
...
Because engine deployment for computing PH is required by at least two [tdaverse](https://github.com/tdaverse/) packages ([{ggtda}](https://tdaverse.github.io/ggtda/) and {tdarec}), deployment functions may be included in the helper package described in the section on Aim 2.
...
- test and report packages developed as part of the other specific aims ({tdarec}, {plt}, etc.);
...
1. Installable upgrade of {ripserr} using the current [Ripser](http://ripser.org/) code base;
...
- {ripserr} repo and CRAN pages do not reference the correct website URL following migration to the [tdaverse](https://github.com/tdaverse) organization.
