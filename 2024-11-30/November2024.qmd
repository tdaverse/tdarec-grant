...
Development is well ahead of schedule.
...
Once completed, JBR will work to expose this feature to the {tdarec} interface.

#### {tdarec} feature requests
...
2. Enable users to tune scale sequences (grids over which functions are vectorized) differently by topological feature dimension and by persistent diagram axis.
...
JBR will work with UIS to address issues that arise from any more substantive changes.

#### {tdarec} engine deployment
...
Eventually, this will be done via dependency on [{phutil}](https://github.com/tdaverse/phutil), in development by AST.
However, it can be completed based on similar tools in [{ggtda}](https://github.com/tdaverse/ggtda) if development of [{phutil}](https://github.com/tdaverse/phutil) is not yet complete, then refactored after initial release.

#### {tdarec} hyperparameter tuners
...
They have also gained proficiency in the calculation---by hand---of persistent homology for small point clouds (distance matrices derived from coordinate matrices).
...
[^ripser-structure]: <https://docs.google.com/document/d/1Mi7PxgvZuYiu_fubsw8J5UYhxCJyWM0UBCa29hJrbsk/> -- We will share the document with an email address upon request.
...
They decided on the latter approach; an export to R via {Rcpp} is drafted in [the `first-transition-to-R` branch](https://github.com/StrongKs/ripser/tree/first-transition-to-R) of KPH's fork of the Ripser repo.
Upon completion, the reorganized and R-exported code will be grafted to a branch of {ripserr}, replacing the current 'ripser_short.cpp' source.